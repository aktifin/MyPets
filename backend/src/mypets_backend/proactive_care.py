...
high-value notice per pet, add due reminders, respect local quiet hours, and leave
persistence/rate limiting to the API layer.
"""
...
def normalize_preferences(raw: Mapping[str, object] | None) -> dict[str, object]:
    ...
    source["quiet_start"] = _parse_clock(str(source.get("quiet_start", "22:00")), "22:00").strftime("%H:%M")
    source["quiet_end"] = _parse_clock(str(source.get("quiet_end", "08:00")), "08:00").strftime("%H:%M")
    source["min_interval_minutes"] = max(15, min(1440, int(source.get("min_interval_minutes", 120))))
    source["max_daily_notices"] = max(1, min(12, int(source.get("max_daily_notices", 3))))
    return source
...
def _low_state_candidate(pet: object) -> dict[str, object] | None:
    ...
    values = [
        (int(_value(pet, "health", 100)), "health", 70, None, "状态需要留意", "打开状态页看看最近变化。"),
        (int(_value(pet, "energy", 100)), "energy", 35, "rest", "有点累了", "让它休息一会儿，会更适合继续互动。"),
        (int(_value(pet, "hunger", 100)), "hunger", 45, "feed", "有点饿了", "现在投喂一次，补充今天的照料。"),
        (int(_value(pet, "cleanliness", 100)), "cleanliness", 45, "clean", "需要整理一下", "清洁一次，让它保持舒适。"),
        (int(_value(pet, "mood", 100)), "mood", 45, "play", "心情有点低", "陪它玩一会儿，增加今天的互动。"),
        (100 - int(_value(pet, "boredom", 0)), "boredom", 35, "play", "有点无聊", "陪它玩一会儿，换个轻松的状态。"),
    ]
        ...
        candidates.append(
            (
                severity,
                {
                    ...
                    "title": f"{name}{title}",
                    "detail": detail,
                    "action_label": "查看状态" if action is None else {
                        "feed": "去投喂",
                        "play": "去陪伴",
                        "clean": "去清洁",
                        "rest": "去休息",
                    ...
                    "care_action": action,
                    "target_section": "pets-section" if action is None else "dashboard-section",
                },
            )
        )
    ...
    *,
    ...
    for pet in pets:
        ...
        if bool(prefs["inactivity_enabled"]):
            ...
            baseline = _aware(last) if last is not None else _aware(_value(pet, "created_at", current))
    ...
    if bool(prefs["reminder_enabled"]):
        for reminder in reminders:
            ...
            if state not in PENDING_REMINDER_STATES or not isinstance(scheduled, datetime):
                continue
            ...
            overdue_minutes = max(0, int((current - scheduled_aware).total_seconds() // 60))
            candidates.append(
                {
    ...
    return sorted(candidates, key=lambda item: (-int(item["priority"]), str(item["notice_key"])))
