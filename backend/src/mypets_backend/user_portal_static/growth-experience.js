...
function installGrowthExperience() {
  ...
  if (dashboardSection && !$("growth-goal-panel")) {
    ...
    headingCopy.append(
      experienceNode("p", "eyebrow", "GROWTH GOAL"),
      experienceNode("h2", "", "下一步成长目标"),
    );
    ...
    action.addEventListener("click", async () => {
      ...
      try {
        const suggestedAction =
          portalGrowthExperienceState.value?.progress?.suggested_action || "play";
        await performPhase1Care(suggestedAction, action);
      } catch (error) {
    ...
    if (daily?.parentElement === dashboardSection) {
      daily.insertAdjacentElement("afterend", panel);
    } else {
      dashboardSection.insertBefore(panel, dashboardSection.querySelector(".two-column"));
    }
  }
  ...
  if (growthDetail && !$("pet-growth-memory-list")) {
    ...
    wrapper.append(
      experienceNode("h3", "", "成长纪念册"),
      experienceNode("p", "hint", "升级、羁绊和阶段变化会保存在这里。"),
    );
...
async function refreshGrowthExperience() {
  ...
  portalGrowthExperienceState.value = await api(
    `/api/v1/pets/${encodeURIComponent(petId)}/growth-experience?limit=30`,
  );
...
function renderGrowthExperience() {
  ...
  $("growth-current-stage").textContent =
    progress.current_stage_label || "成长中";
  $("growth-goal-headline").textContent =
    progress.headline || "继续陪伴即可成长";
  $("growth-goal-detail").textContent =
    progress.detail || "不同照料方式都会积累成长经验。";
  const action = $("growth-goal-action");
  action.textContent = progress.final_stage
    ? "继续轻松玩耍"
    : `去${progress.suggested_action_label || "玩耍"}`;
  const availability =
    typeof dailyCareAvailability === "function"
      ? dailyCareAvailability(progress.suggested_action || "play")
      : null;
  action.disabled = Boolean(availability && !availability.available);
  action.title =
    availability?.reason || "玩耍能够较快积累成长经验，但不要求连续操作。";

  setGrowthProgress("growth-stage-progress", progress.stage_progress_percent, 100);
  $("growth-stage-label").textContent = progress.final_stage
    ? "成熟阶段"
    : `${progress.stage_progress_percent || 0}% · 目标 Lv.${progress.next_stage_target_level}`;
  setGrowthProgress(
    "growth-level-progress",
    progress.growth_level_current,
    progress.growth_level_target,
  );
  $("growth-level-label").textContent =
    `${progress.growth_level_current || 0} / ${progress.growth_level_target || 100} · 还差 ${progress.growth_exp_remaining || 0}`;
  setGrowthProgress(
    "growth-bond-progress",
    progress.bond_level_current,
    progress.bond_level_target,
  );
  $("growth-bond-label").textContent =
    `${progress.bond_level_current || 0} / ${progress.bond_level_target || 80} · 还差 ${progress.bond_exp_remaining || 0}`;
...
function renderGrowthMemories(memories) {
  ...
  memories.forEach((memory) => {
    ...
    copy.append(
      experienceNode("strong", "", memory.title),
      experienceNode("p", "", memory.detail),
      experienceNode("small", "", `${dateText} · ${memory.source_label}`),
    );
...
portalRuntime.registerFeature({
  ...
  label: "成长目标与纪念册",
  order: 110,
  mount: installGrowthExperience,
  onPetContextRefresh: refreshGrowthExperience,
  onRefreshComplete: renderGrowthExperience,
  onSectionEnter: ({ sectionId }) => {
    if (sectionId === "dashboard-section" || sectionId === "pets-section") {
      renderGrowthExperience();
    }
  },
  onCareComplete: renderGrowthExperience,
  onLogout: () => {
    portalGrowthExperienceState.value = null;
    portalGrowthExperienceState.petId = "";
    renderGrowthExperience();
