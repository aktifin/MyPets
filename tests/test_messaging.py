...
from onepic_desktop_pet.messaging import (
    ConversationRecord,
    MessageRecord,
)
...
def _conversation_payload(unread_count: int) -> dict:
    return {
        "conversation_id": "conversation-1",
        "kind": "direct",
        "title": "小波",
        ...
        "peer": _account("account-sender", "sender_user", "小波"),
...
def test_message_drawer_reads_cache_and_emits_user_actions(tmp_path: Path) -> None:
    ...
    assert "💬 1" in drawer.conversation_list.item(0).text()
    assert drawer.message_list.count() == 1
