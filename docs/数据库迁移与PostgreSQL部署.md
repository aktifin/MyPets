...
桌面端 SQLite 仍然只是可重建缓存，不纳入本迁移链。本文所述数据库是 FastAPI 后端的服务端权威数据库。
...
5. 启动新版本应用；
6. 验证 `/health`、登录、宠物列表、消息、提醒和素材发布链路；
7. 恢复流量。

## 四、接管历史 create_all 数据库
...
- 服务端默认值；
- 外键、唯一约束和索引；
- 投稿、制作、D3 部署、视觉身份和版权治理等全部 35 张业务表。
...
- SQLite 从空库升级到 head；
- 检查 35 张业务表和 Alembic 版本表；
- SQLite 降级到 base 后重新升级；
- 历史 `create_all` 数据库结构比对、stamp 和漂移检查；
- PostgreSQL 16 从空库升级、漂移检查、降级和重新升级；
